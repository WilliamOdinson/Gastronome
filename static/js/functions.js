(function ($) {
	...
	function toggleChevron(e) {
		const $icon = $(e.target)
			.prev('.card-header')
			.find("i.indicator.material-icons");
	
		if ($icon.length) {
			const isExpanded = $(e.target).hasClass('show');
			// show means now expanded -> should display 'remove'
			$icon.text(isExpanded ? 'remove' : 'add');
		}
	}
	$('.accordion_2').on('shown.bs.collapse hidden.bs.collapse', toggleChevron);
