...
echo ">>> Importing business data, 150346 entires (~22 min for batch = 500)"
python manage.py import_business "$DATA_DIR/yelp_academic_dataset_business.json"

echo ">>> Importing user data, 1987897 entires (~4 min for batch = 5,000)"
python manage.py import_user "$DATA_DIR/yelp_academic_dataset_user.json"

echo ">>> Importing checkin data, 131930 entires (~14 min for batch = 5,000)"
python manage.py import_checkin "$DATA_DIR/yelp_academic_dataset_checkin.json"

echo ">>> Importing photo data,  entires (~0.1 min for batch = 10,000)"
...
echo ">>> Importing tips, 908915 entires (~2 min for batch = 10,000)"
